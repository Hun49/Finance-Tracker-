...
export default function IncomePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["income"],
    queryFn: () => api<{ incomes: Income[] }>("/income"),
  });

  const addMutation = useMutation({
    mutationFn: (input: TransactionFormData) => api("/income", jsonPost(input)),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api(`/income/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["income"] });
      queryClient.invalidateQueries({ queryKey: ["reports"] });
    },
  ...
  const total = incomes.reduce((sum, income) => sum + Number(income.convertedAmount), 0);

  return (
    <div className="space-y-5">
      <div className="glass-card flex flex-wrap items-center justify-between gap-4 rounded-3xl p-6">
        <div>
          <h1 className="text-2xl font-black tracking-tight">Income</h1>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            One-time, daily, weekly, or monthly. Extra income any day of the month.
          </p>
        </div>
        <div className="text-right">
          <p className="text-sm text-slate-500 dark:text-slate-400">Total recorded</p>
          <p className="text-3xl font-black text-emerald-600 dark:text-emerald-400">{formatMoney(total, currency)}</p>
        </div>
      </div>
      ...
      {!adding && !editing ? (
        <div className="flex gap-3">
          <button
            ...
            className="rounded-full bg-emerald-600 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-emerald-600/25 transition hover:bg-emerald-500"
          >
            Add income
          </button>
          <Link
        ...
        <div className="glass-card rounded-3xl p-6">
          <div className="mb-5 flex items-center justify-between">
            <h2 className="font-black">{editing ? "Edit income" : "Record income"}</h2>
            <button
              onClick={() => {
                setAdding(false);
                setEditing(null);
          ...
          <TransactionForm
            ...
            submitLabel={editing ? "Save changes" : "Save income"}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                      {income.category} • {formatDate(income.date)} {income.frequency !== "ONCE" ? `• ${income.frequency.toLowerCase()}` : ""}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <p className="font-black text-emerald-600 dark:text-emerald-400">
                    +{formatMoney(Number(income.convertedAmount), currency)}
                  </p>
